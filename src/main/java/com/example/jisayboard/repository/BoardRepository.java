...
public interface BoardRepository extends JpaRepository<Board, Long> {
	 ...
	 */
	@Query(value = "select b, w, count(r) from Board b LEFT JOIN b.writer w LEFT JOIN Reple r ON r.board = b group by b",
			countQuery = "select count(b) From Board b")
	Page<Object[]> getBoardWithRepleCount(Pageable pageable);

	@Query(value = "SELECT b, w, COUNT(r) FROM Board b LEFT JOIN b.writer w LEFT JOIN Reple r ON r.board = b WHERE b.title LIKE %:search% OR b.content LIKE %:search% GROUP BY b",
			countQuery = "SELECT COUNT(b) FROM Board b WHERE b.title LIKE %:search% OR b.content LIKE %:search%")
	Page<Object[]>  findByTitleContaining(String search, Pageable pageable);
	...
	@Query("select b, r From Board b LEFT Join Reple r ON r.board=b where b.bno=:bno")
	List<Object[]> getBoardWithReple(@Param("bno") Long bno);
	...
	@Query("select b , w, count(r) from Board b left join b.writer w left join Reple r on r.board = b where b.bno =:bno")
